...
namespace Scanner
{
    delegate void MatchFoundCallback(string searchingWord, int line, int column, string lineText, string warning);
    ...
    class MatchFinder
    {
        ...
        public MatchFinder(string searchingWord)
        {
            if (string.IsNullOrEmpty(searchingWord))
            {
                throw new ArgumentNullException("searchingWord");
            }

            _termIndex = CreateSortedTermIndex(searchingWord);
        ...
        class TermMatch : MatchBase
        {
            ...
            public void LineCompleted(string lineText)
            {
                ...
                {
                    _finder.RemoveAllMatchesInRangeExceptOne(Line, Column, Line, Column + Term.SearchingWord.Length - 1, this);
                }
            }
            ...
            public override void AddChar(char c)
            {
                if (!DiscardPending)
                {
                    ...
                    else
                    {
                        ...
                        if (char.ToLowerInvariant(Term.SearchingWord[_matchedChars]) == char.ToLowerInvariant(c))
                        {
                            ++_matchedChars;
                            if (IsMatched)
                            {
            ...
            {
                get { return !DiscardPending && _matchedChars == Term.SearchingWord.Length; }
            }
        ...
        class ExclusionMatch : MatchBase
        {
            ...
            public bool Excludes(TermMatch match)
            {
                return match.Term == _exclusion.Term &&
                    ...
                    RangeContains(_line, _column, _line, _column + _exclusion.Text.Length - 1, match.Line, match.Column + match.Term.SearchingWord.Length - 1);
            }
        ...
        class MatchableTerm
        {
            ...
            public MatchableTerm(string searchingWord, int originalTableIndex, int originalTermIndex)
            {
        ...
        private static List<MatchableTerm> CreateSortedTermIndex(string searchingWord)
        {
            List<MatchableTerm> index = new List<MatchableTerm>();
            int tableIndex = 0;

            index.Add(new MatchableTerm(searchingWord, 0, 0));
            ...
            {
                if ((i > 0 && string.Compare(index[i].SearchingWord, index[i - 1].SearchingWord, StringComparison.OrdinalIgnoreCase) == 0) ||
                    (i < index.Count - 1 && string.Compare(index[i].SearchingWord, index[i + 1].SearchingWord, StringComparison.OrdinalIgnoreCase) == 0))
                {
        ...
        {
            int result = CompareStringsWithLongestFirst(x.SearchingWord, y.SearchingWord);
            if (result == 0)
            {
            ...
            foreach (TermMatch match in ConfirmedMatches)
            {
                if (callback != null)
                {
                    ...
                    if (match.Term.HasDuplicates)
                    {
                        ...
                        warning = string.Format(CultureInfo.CurrentUICulture, Resources.DuplicateTermWarning, match.Term.SearchingWord, tableList.ToString());
                    }
                    callback(match.Term.SearchingWord, match.Line, match.Column, match.LineText, warning);
                }
                match.MarkForDiscard();
            }
        }
        ...
        private IEnumerable<MatchableTerm> GetDuplicatesOf(MatchableTerm sourceTerm)
        {
            return _termIndex.Where(
                possibleDup => string.Compare(sourceTerm.SearchingWord, possibleDup.SearchingWord, StringComparison.OrdinalIgnoreCase) == 0);
        }
        ...
        private IEnumerable<MatchableTerm> TermsStartingWith(char c)
        {
            return BinarySearch(_termIndex,
                term => char.ToLowerInvariant(term.SearchingWord[0]) - char.ToLowerInvariant(c));
        }
        ...
        void RemoveAllMatchesInRangeExceptOne(int lineStart, int columnStart, int lineEnd, int columnEnd, TermMatch matchToSave)
        {
            foreach (TermMatch match in PartiallyMatchedTerms)
            {
                if (match != matchToSave && RangesOverlap(lineStart, columnStart, lineEnd, columnEnd, match.Line, match.Column, match.Line, match.Column + match.Term.SearchingWord.Length - 1))
                {
                    match.MarkForDiscard();
                }
            }
